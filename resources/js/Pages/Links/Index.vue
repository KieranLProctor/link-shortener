        ...
        <template #header>
            <div class="flex justify-between">
                ...
                <button
                    ...
                    @click="openCreateModal()">Shorten Link</button>
            </div>
        </template>
...
export default {
    ...
    data() {
        return {
            stats: [
                { name: 'Total Links', stat: this.links.length },
                { name: 'Total Visitors', stat: this.links.length > 0 ? this.links.map(link => link.visitors.length).reduce((prev, next) => prev + next) : 0 },
                { name: 'Not sure what to put in here', stat: 0 }
            ],
